...
## 2026-09-22 — Eighth tracked target: Leopold Aschenbrenner

The project conversation added Leopold Aschenbrenner as the eighth tracked person and the sixth SEC 13F-backed manager. His 13F is filed by Situational Awareness LP through a filing agent (accession `0000935836`), so the collecting code still requires the submitter identity and the cover-page manager name to match rather than trusting the agent accession. The filing mixes common stock, PUT, and CALL positions, and the detail view keeps the same stock/PUT/CALL separation used for Michael Burry; no real-time or performance wording is introduced.

Front-end work reuses the existing patterns only: the home card list, the home support badge and card count, the desktop sidebar and the narrow-screen menu, and the shared `SecManagerDetail` screen union all gained `aschenbrenner`, and the new detail route is `/main/gurus/leopold-aschenbrenner` on the shared SEC 13F coordinator. The access endpoint `/api/sync/aschenbrenner`, the operator entry `/api/internal/sync/aschenbrenner`, migration `supabase/migrations/202609220002_aschenbrenner.sql`, and the disabled-by-default `guru_tracker_aschenbrenner_hourly` job at minute 43 follow the existing per-target convention, and the existing seven targets keep their styles and behavior.

Updated [tracked people](decisions/tracked-people.md), [source provenance](concepts/source-provenance.md), [product direction](product/product-direction.md), [the index](index.md), `.omp/AGENTS.md`, and README. Applied the remote migration and collected official filing `0000935836-26-000418`: report date 2026-06-30, filed 2026-08-14, 26 rows totaling USD 20,242,292,228. Initial storage returned `updated`; reprocessing returned `unchanged`, preserving version 1 and one event. The local Next.js API returned 401 without authentication, 403 for a foreign Origin, and 200 `unchanged` with valid internal authentication. Type/Biome checks and all 64 tests passed. Isolated Chromium rendered the actual new detail with the collected snapshot at 1440/390/320px, light/dark themes, eight home cards, and the mobile navigation without horizontal overflow. Authentication, ticker lookup, and routing were substituted, so real-account navigation and progress completion remain unverified. The minute-43 Cron job is registered inactive pending app deployment and deployed API verification. No human-owned raw evidence was modified.
