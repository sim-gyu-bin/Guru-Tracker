-- Michael Burry(Scion Asset Management, LLC) SEC 13F의 시간별 동기화 운영 설정.
...
$command$;
  ...
  -- ARK(7분)·하원 PTR(13분)과 겹치지 않게 19분에 두어 SEC 요청이 한 시각에 몰리지 않게 한다.
  select jobid into v_job_id from cron.job where jobname = 'guru_tracker_burry_hourly';
  if v_job_id is not null then
    ...
    perform cron.schedule('guru_tracker_burry_hourly', '19 * * * *', v_command);
  end if;

  select count(*) into v_duplicates from cron.job where jobname like 'guru_tracker_burry%';
  if v_duplicates > 1 then
    raise exception 'guru_tracker_burry%% 이름의 cron job이 %개 있습니다. 중복 job은 여기서 삭제하지 않으니 운영자가 직접 확인하세요.', v_duplicates;
  end if;
end $outer$;

-- 확인용(선택): 실행 job과 최근 응답 상태를 조회한다. 응답 본문·헤더는 남기지 않는다.
--   select jobid, jobname, schedule, active from cron.job where jobname = 'guru_tracker_burry_hourly';
