-- laffont/gerstner/tepper/aschenbrenner 신규 job만 관리한다. 기존 다른 관리자 job·Vault 값은 변경하지 않는다.
...
-- Leopold Aschenbrenner(Situational Awareness LP) SEC 13F의 시간별 동기화 운영 설정.
-- 마이그레이션에서 자동 활성화하지 않는다. 운영자가 Vault 값을 넣은 뒤 이 파일을 직접 실행한다.
-- 비밀값은 Vault에서 실행 시점에만 읽고 이 파일·cron.job 명령·로그에 문자열로 남기지 않는다.
-- Vault 항목은 Supabase Dashboard의 Project Settings > Vault 화면에서 추가한다.
-- SQL Editor에 비밀값을 넣은 create_secret 문을 실행하지 않는다. SQL 실행 이력에 그대로 남는다.
...
begin
  ...
  -- 대상은 최신 13F 1건이므로 한 번만 호출한다. 응답은 60초 한도이며 엔드포인트의 maxDuration과 같다.
  -- Leopold Aschenbrenner는 Stanley와 다른 lease를 쓰므로 두 동기화는 서로를 막지 않는다. 같은 대상의 중복 실행만 busy로 끝난다.
  v_command := $command$
select net.http_post(
  url := rtrim(btrim((select decrypted_secret from vault.decrypted_secrets where name = 'guru_tracker_base_url')), '/')
    || '/api/internal/sync/aschenbrenner',
  headers := jsonb_build_object(
    'Content-Type', 'application/json',
    'Authorization', 'Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'guru_tracker_sync_secret')
  ),
...
$command$;
  ...
  -- 기존 ARK(7분)·PTR(13분)·Burry(19분)·Laffont(25분)·Gerstner(31분)·Tepper(37분)와 겹치지 않게 매시 43분에 실행한다.
  select jobid into v_job_id from cron.job where jobname = 'guru_tracker_aschenbrenner_hourly';
  if v_job_id is not null then
    perform cron.alter_job(
      job_id := v_job_id,
      schedule := '43 * * * *',
    ...
    v_job_id := cron.schedule('guru_tracker_aschenbrenner_hourly', '43 * * * *', v_command);
    -- 등록과 비활성화를 같은 transaction에서 끝내어 미확인 API 호출을 막는다.
    perform cron.alter_job(job_id := v_job_id, active := v_endpoint_verified);
  end if;

  select count(*) into v_duplicates from cron.job where jobname like 'guru_tracker_aschenbrenner%';
  if v_duplicates > 1 then
    raise exception 'guru_tracker_aschenbrenner%% 이름의 cron job이 %개 있습니다. 중복 job은 여기서 삭제하지 않으니 운영자가 직접 확인하세요.', v_duplicates;
  end if;
end $outer$;

-- 확인용(선택): 실행 job과 최근 응답 상태를 조회한다. 응답 본문·헤더는 남기지 않는다.
--   select jobid, jobname, schedule, active from cron.job where jobname = 'guru_tracker_aschenbrenner_hourly';
--   select id, status_code, timed_out, created from net._http_response order by created desc limit 12;
-- Authorization 헤더는 pg_net이 전송할 때까지 net.http_request_queue에 남는다. 이 표를 조회·복사해 로그나 문서로 남기지 않는다.
