...
begin
  ...
  -- 펀드 6개를 한 job에서 각각 호출한다. 요청은 큐에 들어가므로 서로의 타임아웃을 기다리지 않고,
  -- 응답은 각 60초 한도이며 엔드포인트의 maxDuration과 같다. 실패한 펀드는 다음 시간에 다시 시도한다.
  v_command := $command$
with funds(fund) as (
...
select net.http_post(
  url := rtrim(btrim((select decrypted_secret from vault.decrypted_secrets where name = 'guru_tracker_base_url')), '/')
    || '/api/internal/sync/ark?fund=' || f.fund,
...
$command$;
  ...
  select jobid into v_job_id from cron.job where jobname = 'guru_tracker_ark_hourly';
  if v_job_id is not null then
    ...
    perform cron.schedule('guru_tracker_ark_hourly', '7 * * * *', v_command);
  end if;

  select count(*) into v_duplicates from cron.job where jobname like 'guru_tracker_ark%';
  if v_duplicates > 1 then
    raise exception 'guru_tracker_ark%% 이름의 cron job이 %개 있습니다. 중복 job은 여기서 삭제하지 않으니 운영자가 직접 확인하세요.', v_duplicates;
  end if;
end $outer$;

-- 확인용(선택): 실행 job과 최근 응답 상태를 조회한다. 응답 본문·헤더는 남기지 않는다.
--   select jobid, jobname, schedule, active from cron.job where jobname = 'guru_tracker_ark_hourly';
