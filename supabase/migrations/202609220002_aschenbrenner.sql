-- Leopold Aschenbrenner(Situational Awareness LP) 전용 캐시·펜싱 lease·원자 스냅샷 회전. Burry 계약을 그대로 적용한다.

-- Leopold Aschenbrenner(Situational Awareness LP) 전용 캐시. 현재/직전만 보존하며 원문 객체는 별도 승인 없이 삭제하지 않는다.
-- Stanley와 RPC·테이블 이름만 다르고 펜싱 토큰·lease·검증·원자 회전 규칙은 같은 계약을 따른다.
-- 기존 관리자 테이블·버킷·원문은 이 마이그레이션에서 변환·삭제하지 않는다.
create table public.aschenbrenner_state (
...
alter table public.aschenbrenner_state enable row level security;
alter table public.aschenbrenner_events enable row level security;
revoke all on public.aschenbrenner_state, public.aschenbrenner_events from public, anon, authenticated, service_role;
grant select on public.aschenbrenner_state to service_role;
...
create function public.aschenbrenner_acquire() returns text
language plpgsql security definer set search_path = '' as $$
declare s public.aschenbrenner_state;
begin
  select * into strict s from public.aschenbrenner_state where id for update;
  ...
  update public.aschenbrenner_state set fence = fence + 1,
...
create function public.aschenbrenner_fail(p_fence text, p_error text) returns boolean
language plpgsql security definer set search_path = '' as $$
begin
  update public.aschenbrenner_state set lease_until = null,
...
-- CIK·제출자 이름·원문 접두사를 Leopold Aschenbrenner 값으로 고정해 다른 관리자의 스냅샷 커밋을 거부한다.
-- 이 대상의 13F는 제출 대행 접수(accession 앞자리 0000935836)로 올라오므로 제출자 신원은 accession이 아니라
-- submissions 제출자 CIK·이름과 표지 filingManager.name으로만 판정한다.
create function public.aschenbrenner_commit(p_fence text, p_snapshot jsonb, p_document_hash text, p_normalized_hash text, p_raw_prefix text) returns text
language plpgsql security definer set search_path = '' as $$
declare s public.aschenbrenner_state; next_version bigint;
begin
  select * into strict s from public.aschenbrenner_state where id for update;
  ...
  if p_snapshot is null or p_document_hash is null or p_normalized_hash is null or p_raw_prefix is null
    ...
    or p_snapshot->>'cik' is distinct from '0002045724'
    or p_snapshot->>'managerName' is distinct from 'Situational Awareness LP'
    ...
    or p_raw_prefix is distinct from ('aschenbrenner/' || p_document_hash)
  ...
  if s.document_hash = p_document_hash or s.normalized_hash = p_normalized_hash then
    ...
    update public.aschenbrenner_state set current_snapshot = p_snapshot || jsonb_build_object('version', s.dataset_version),
  ...
  update public.aschenbrenner_state set previous_snapshot = current_snapshot, previous_raw_prefix = raw_prefix,
    ...
    last_success_at = clock_timestamp(), last_error = null, lease_until = null where id;
  insert into public.aschenbrenner_events(event_key, dataset_version, accession, document_hash, normalized_hash)
    values ('aschenbrenner:' || next_version::text || ':' || p_document_hash, next_version, p_snapshot->>'accession', p_document_hash, p_normalized_hash);
  return 'updated';
end $$;

revoke all on function public.aschenbrenner_acquire() from public, anon, authenticated;
revoke all on function public.aschenbrenner_fail(text,text) from public, anon, authenticated;
revoke all on function public.aschenbrenner_commit(text,jsonb,text,text,text) from public, anon, authenticated;
grant execute on function public.aschenbrenner_acquire() to service_role;
grant execute on function public.aschenbrenner_fail(text,text) to service_role;
grant execute on function public.aschenbrenner_commit(text,jsonb,text,text,text) to service_role;
