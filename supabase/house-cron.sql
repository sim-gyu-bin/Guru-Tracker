-- Nancy Pelosi 하원 PTR 최신 제출 1건의 시간별 동기화 운영 설정.
...
begin
  ...
  -- 대상은 최신 PTR 1건이므로 펀드별 반복 없이 한 번만 호출한다. 응답은 60초 한도이며
  -- 엔드포인트의 maxDuration과 같다. 다른 동기화가 lease를 잡고 있으면 busy로 끝나고 다음 시간에 다시 시도한다.
  v_command := $command$
...
$command$;
  ...
  select jobid into v_job_id from cron.job where jobname = 'guru_tracker_house_hourly';
  if v_job_id is not null then
    ...
    perform cron.schedule('guru_tracker_house_hourly', '13 * * * *', v_command);
  end if;

  select count(*) into v_duplicates from cron.job where jobname like 'guru_tracker_house%';
  if v_duplicates > 1 then
    raise exception 'guru_tracker_house%% 이름의 cron job이 %개 있습니다. 중복 job은 여기서 삭제하지 않으니 운영자가 직접 확인하세요.', v_duplicates;
  end if;
end $outer$;

-- 확인용(선택): 실행 job과 최근 응답 상태를 조회한다. 응답 본문·헤더는 남기지 않는다.
--   select jobid, jobname, schedule, active from cron.job where jobname = 'guru_tracker_house_hourly';
