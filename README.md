...
관리자 메뉴와 페이지 이름은 **운영 관리**이며, 내부를 **가입 승인 / 동기화 상태** 탭으로 나눕니다. `/admin`은 가입 승인, `/admin?tab=sync`는 동기화 상태를 표시합니다. 선택은 URL에 유지되므로 새로고침과 뒤로·앞으로 이동에도 복원됩니다. 승인 메일·처리 결과의 `request`·`intent`·`revision`·`result`가 있으면 가입 승인 탭을 우선하고, 탭을 바꿀 때 해당 안내 쿼리를 제거합니다. 알 수 없거나 중복된 `tab` 값은 가입 승인으로 표시합니다.

탭은 기존 테마·반응형 레이아웃을 재사용하며 가입 승인에 대기 건수를 표시합니다. 화살표·Home·End로 포커스를 옮기고 Enter·Space로 선택하며, 선택 변경은 공통 상단 이동 프로그래스를 사용합니다. 비활성 패널의 승인 폼은 표시·포커스 대상에서 제외합니다. 관리자 검증과 승인 server action은 기존 동작을 유지합니다.
...
프로젝트 폰트는 한글·영문·숫자 모두 **Pretendard Variable**로 통일하며 숫자는 `tabular-nums`로 정렬합니다. 사용 중인 Pretendard 자산과 라이선스를 보관하며, 미사용 Space Grotesk 자산과 라이선스는 사용자 승인으로 제거했습니다. 본문 16px, 조회 홈·여덟 상세의 페이지 제목 30px/600, 섹션 제목 20px/600, 카드 제목과 도입 설명 18px, 표·보조 설명 14px를 사용합니다. 모바일 탐색 항목은 24px/500입니다. 초기 테마는 시스템을 따르고 수동 선택을 저장합니다. 수동 선택 전에는 OS 변경을 따릅니다. 사전 스크립트가 첫 화면의 색상 모드를 정하고 차트·브라우저 색도 같은 모드를 따릅니다.
...
Nancy Pelosi의 자산별 활동 요약 막대도 공통 초록→연두 그라데이션 토큰을 사용합니다. 다크에서는 약하게 발광하고 라이트에서는 짙은 색만 사용하며, 신고 건수와 최다 건수 기준 상대 길이는 그대로 유지합니다.

메인 조회 카드도 이름은 공통 전경색, 설명은 기본 보조 텍스트 색으로 표시하고 이니셜·출처 배지에는 초록 포인트와 약한 다크 발광을 사용합니다. 상단 안내는 제목·지원 배지 아래 전체 너비를 사용하며 한국어 단어 중간에서 줄바꿈하지 않습니다.
...
내부 이동 링크는 `GuruLink`, 명령형 이동은 `useGuruRouter`를 사용합니다. 루트의 `NavigationProvider`가 React transition을 소유하고 화면 최상단에 2px 진행 바를 즉시 표시합니다. 모바일 메뉴나 링크가 사라져도 진행 상태를 유지하며 브라우저 뒤로·앞으로, 관리자 작업의 페이지 전환, 같은 탭의 일반 링크·폼 문서 이동도 감지합니다. 새 탭·다운로드·동일 URL·페이지 내 해시 이동은 제외합니다. `refresh()`와 `prefetch()`는 진행 바를 시작하지 않으므로 백그라운드 갱신은 조용하게 유지됩니다.

진행 바는 8%부터 시작해 400ms마다 남은 연출 구간의 일부를 채우며 94%에 점차 접근합니다. 내부 이동 완료 시에만 100%로 늘어난 뒤 페이드아웃하고 취소는 성공 연출 없이 숨깁니다. 실제 전송량·완료율을 뜻하지 않으며 모션 감소 설정에서는 보간을 생략합니다. 문서 이동은 교체 전까지 표시하고 이후에는 새 문서가 표시를 소유합니다. Navigation API 미지원 브라우저에서는 문서 이동 취소 감지가 Escape·페이지 복원 신호에 한정됩니다.
...
여섯 SEC 상세와 ARK의 여섯 펀드는 동일한 큰 도넛 디자인을 사용합니다. 상위 5개 합산 비중은 위에, 중앙에는 해당 인물 사진을, 아래에는 한글 이름을 표시합니다. ARK는 캐시 우드 사진을 사용하되 부제의 펀드명과 개인 계좌가 아니라는 안내를 유지합니다. 각 조각은 독립적으로 왼쪽 기본색에서 오른쪽 밝은색으로 흐르는 선형 그라데이션을 쓰며, 기타의 색 변화는 더 약하게 적용합니다. 조각 사이에는 1.2° 간격을 두고 꼭짓점은 살짝 둥글리되 선 테두리·사진 프레임은 두지 않습니다. 간격을 제외한 각도를 원래 비중대로 배분하며 라벨·목록의 실제 비중은 바꾸지 않습니다. 사진은 가운데 공간을 넓게 채우며, 도넛 전체와 원형 사진 뒤에만 부드러운 어두운 그림자를 적용합니다. 모든 조각은 티커·비중 두 줄을 유지합니다. 조각의 중간 각도와 도넛 두께의 중간 위치에 글자 묶음을 가운데 정렬하고, 웹폰트의 실제 글자 크기를 측정해 18px부터 최소 12px까지 단계적으로 줄입니다. 중앙에서 두 줄이 들어가지 않을 때만 라벨 전체를 조각 방향에 가까운 위·아래·좌·우 여백으로 옮깁니다. 바깥 라벨은 겹치지 않도록 간격을 조정하며 라벨 공간에 맞춰 도넛 크기를 정합니다. 연결선은 꺾임 없는 직선 하나이며, 같은 방향의 작은 열린 화살촉이 해당 조각의 바깥 호 중앙을 가리킵니다. 선과 글자 사이에는 여백을 둡니다. 확인된 티커가 없으면 순번 대신 축약한 발행사 이름을 씁니다. 전체 발행사·티커·비중·금액·SEC의 PUT/CALL 정보는 상시 목록에서 확인할 수 있습니다. SEC·ARK 차트는 별도 툴팁 없이 조각 라벨과 상시 목록으로 정보를 제공합니다.

버리 차트는 `autoRotate`를 켜서 큰 반지름부터 0·90·180·270도와 45·135·225·315도 시작각을 차례로 검사합니다. 옆에 몰린 바깥 라벨을 상하 여백에 배치할 수 있는지 먼저 확인하고, 모든 방향에서 공간이 부족할 때만 도넛을 줄입니다. 비중·조각 순서·각도 폭은 유지하며 다른 인물은 기존 0도 방향을 유지합니다. 버리 사진은 원본 파일을 바꾸지 않고 원형 클립 안에서 1.8배 확대하고 세로 초점을 30%로 옮겨 얼굴을 중심에 둡니다.

인물 사진은 사용자 요청으로 다음 공개 페이지에서 확보해 `public/`에 보관합니다. 화면의 출처 문구는 사용자 요청으로 생략하고 이 문서에 기록합니다. **기존 SEC·ARK 인물 사진의 재사용 허가는 확인하지 않았으며 비상업 용도나 출처 표기만으로 허가가 생기지는 않습니다.** 펠로시 사진의 출처와 공개 라이선스 분류는 표 아래에 별도로 기록합니다.
...
| 낸시 펠로시 | `nancy-pelosi.webp` | [미국 하원 공식 프로필](https://pelosi.house.gov/biography) · 2019년 의장실 공식 사진, John Harrington 촬영 |

펠로시 사진은 공식 프로필이 연결한 [원본](https://pelosi.house.gov/sites/evo-subsites/pelosi-evo.house.gov/files/documents/Portrait%20Photo%20Compressed.jpg)을 480×600 WebP로 최적화했습니다. [Wikimedia Commons의 동일 사진 기록](https://commons.wikimedia.org/wiki/File:Official_photo_of_Speaker_Nancy_Pelosi_in_2019.jpg)은 `PD-USGov-Speaker` 분류와 인격권 관련 주의를 명시합니다. 해당 기록의 메타데이터에는 촬영자 저작권 표기도 남아 있으므로 모든 이용 형태의 권리까지 확인한 것으로 단정하지 않습니다.
...
펠로시 활동 요약에는 도넛 대신 원형 프로필 사진을 결합합니다. 761px 이상에서는 왼쪽 176px 사진과 이름, 오른쪽 가로 지표 3개와 자산별 막대를 표시합니다. 그보다 좁은 화면은 80px 사진·이름 행 아래에 지표와 막대를 쌓습니다. 사진 추가로 집계나 PTR의 의미는 바뀌지 않으며, 빈 상태에서도 0건 안내를 유지합니다.
...
- Scion Asset Management, LLC, CIK `0001649339`를 고정 검증합니다. [공식 submissions](https://data.sec.gov/submissions/CIK0001649339.json)과 [정보표](https://www.sec.gov/Archives/edgar/data/1649339/000164933925000007/infotable.xml)를 실제 수집하여 기준일 **2025-09-30**, 제출일 **2025-11-03**, 8행(주식 등 4·PUT 2·CALL 2)을 확인했습니다. 현재 보유 여부를 뜻하지 않습니다.
