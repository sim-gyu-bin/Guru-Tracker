...
import { syncArk } from "@/server/ark";

export const runtime = "nodejs";
export const maxDuration = 60;

/** 운영자·Supabase Cron용 진입점이다. 서버 비밀값이 없으면 닫고, 접근 갱신과 같은 멱등 조정자를 펀드별로 호출한다. */
export async function POST(request: Request) {
  ...
  const requested = new URL(request.url).searchParams.get("fund");
