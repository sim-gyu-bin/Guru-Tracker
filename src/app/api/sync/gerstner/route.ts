...
/**
 * 같은 출처 화면의 오래된 Brad Gerstner SEC 13F 캐시만 보완한다.
 * 임의 대상·원문 URL을 받지 않고 수집 대상은 서버의 "gerstner" 설정으로 고정되며 DB lease는 내부 경로와 공유한다.
 */
export async function POST(request: Request) {
  ...
  const result = await syncSec("gerstner");
